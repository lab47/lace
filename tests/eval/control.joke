...
(deftest test-while
  (is (= nil (while nil (throw (ex-info "never" {})))))
  (is (= [0 nil]
         ;; a will dec to 0
         ...
         (let [a (atom 3)
               w (while (pos? @a)
                   (swap! a dec))]
           [@a w])))
  (is (thrown? Error (while true (throw (ex-info "expected to throw" {})))))
  )
...
(deftest test-case
  ...
  (testing "throws Error if no match"
    (is (thrown-with-msg?
  ...
  ;;   ;; (is (= :1 (case 1N 1 :1 :else))))  ;; currently broken in Joker
  ...
  (testing "non-equivalence of chars and nums"
    ...
    (are [result input] (= result (case input \a :a :else))
      ...
      :else 97N
      :a (char 97)))
  ;; Joker doesn't detect duplicate case test constants:
  ...
  ;; Joker doesn't pass the "boom" test, below; but I (jcburley) am not sure of the implications:
  ;; (testing "test constants are *not* evaluated"
  ;;   (let [test-fn
  ;;         ;; never write code like this...
  ;;         #(case %
  ;;                (throw (ex-info "boom" {})) :piece-of-throw-expr
  ;;                :no-match)]
  ;;     (are [result input] (= result (test-fn input))
