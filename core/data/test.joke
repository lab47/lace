...
(def ^:dynamic
  ^{:doc "Bound to an atom of a map in test-ns"
    :added "1.0"
    :tag Atom}
  *report-counters* nil)

(def ^:const
  ^{:doc "Used to initialize *report-counters*"
    :added "1.0"}
  *initial-report-counters* {:test 0, :pass 0, :fail 0, :error 0})

(def ^:dynamic
  ^{:doc "Bound to hierarchy of vars being tested"
    :added "1.0"}
  *testing-vars* (list))

(def ^:dynamic
  ^{:doc "Bound to hierarchy of \"testing\" strings"
    :added "1.0"}
  *testing-contexts* (list))

(def ^:dynamic
  ^{:doc "PrintWriter for test reporting output"
    :added "1.0"}
  *test-out* *out*)
...
(defn inc-report-counter
  ...
  (when *report-counters*
    (get (swap-vals! *report-counters* update-in [name] (fnil inc 0)) 1)))
...
;; 2018-12-25 jcburley: Haven't investigated what Joker might already
;; have to offer for stacktrace outputs. For now, provide no info.
...
(defmulti assert-expr
  "Don't call this; add methods to extend the 'is' macro."
  {:added "1.0"}
  (fn [msg form]
    (cond
...
(defmacro are
  ...
  (if (or
    ...
    (throw (ex-info (str "IllegalArgumentException: " "The number of args doesn't match are's argv.") {}))))
