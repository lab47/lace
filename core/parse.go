...
func parseParams(env *Env, params any) ([]Symbol, bool, error) {
	...
	for i := 0; i < v.count; i++ {
		...
		if Equals(env, criticalSymbols.amp, sym) {
			if v.count > i+2 {
				ro := v.at(i + 2)
				s, err := ToString(env, ro)
				if err != nil {
					return nil, false, err
				}
...
func addArity(fn *FnExpr, sig Seq, ctx *ParseContext) error {
	...
	arity := FnArityExpr{
		...
		args:       args,
		body:       bodye,
		taggedType: ctx.Env.ResolveTypeFroMeta(meta),
	}
	if isVariadic {
		if fn.variadic != nil {
...
func parseCatch(obj any, ctx *ParseContext) (*CatchExpr, error) {
	...
	if !IsSymbol(excSymbol) {
		s, err := ToString(ctx.Env, excSymbol)
		if err != nil {
			return nil, err
		}
