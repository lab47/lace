...
func Generate(name, laceName string, base, output string) error {
	...
	*/
	...
	spew.Dump(tpkg.Name(), pkg.ImportPath, pkgs[0].Name)
	...
	fnprint(&buf, pkgName, files, ast.Fun)
	fmt.Fprintln(&buf, "},")
	fmt.Fprintln(&buf, "")
...
func print(w io.Writer, pkgName string, files []*ast.File, kind ast.ObjKind, format string) {
	type ent struct {
		name, doc string
	}
...
func fnprint(w io.Writer, pkgName string, files []*ast.File, kind ast.ObjKind) {
	var fns []string
	...
	for _, f := range files {
		for name, object := range f.Scope.Objects {
			if object.Kind == kind && ast.IsExported(name) {
				...
				arity := strings.Join(args, `,`)
				fns = append(fns,
					fmt.Sprintf("\t\"%s\": {Doc: %q, Args: []pkgreflect.Arg{%s}, Tag: \"%s\", Value: reflect.ValueOf(%s.%[1]s)},\n", fn.Name.Name, strings.TrimSpace(fn.Doc.Text()), arity, rt, pkgName),
				)
			}
		}
	}
