...
func InternsOrThunks(env *Env, ns *Namespace) {
	...
	ns.ResetMeta(MakeMeta(nil, `Implements utility routines for manipulating filename paths.`, "1.0"))

	ns.InternVar(env, "list-separator", list_separator_,
		MakeMeta(
			nil,
			`OS-specific path list separator.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "separator", separator_,
		MakeMeta(
			nil,
			`OS-specific path separator.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))
  ...
  Calls clean on the result.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "abs?", isabs_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Reports whether the path is absolute.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("Boolean")))

	ns.InternVar(env, "base", base_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
  ...
  entirely of separators, returns a single separator.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "clean", clean_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
...
If the result of this process is an empty string, returns the string ".".`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "dir", dir_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
  ...
  returns a single separator. The returned path does not end in a separator unless it is the root directory.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "eval-symlinks", eval_symlinks_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
  ...
  Calls clean on the result.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))
	...
	ns.InternVar(env, "ext", ext_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Returns the file name extension used by path. The extension is the suffix beginning at the final dot
  in the final element of path; it is empty if there is no dot.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "file-seq", file_seq_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("root"))),
			`Returns a seq of maps with info about files or directories under root.`, "1.0"))

	ns.InternVar(env, "from-slash", from_slash_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Returns the result of replacing each slash ('/') character in path with a separator character.
  Multiple slashes are replaced by multiple separators.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "glob", glob_,
		MakeMeta(
  ...
  Throws exception when pattern is malformed.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("[String]")))
  ...
  the result is a UNC path if and only if the first path element is a UNC path.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "matches?", ismatches_,
		MakeMeta(
  ...
  On Windows, escaping is disabled. Instead, '\' is treated as path separator.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("Boolean")))

	ns.InternVar(env, "rel", rel_,
		MakeMeta(
  ...
  the property that path = dir+file.`, "1.0"))
	...
	ns.InternVar(env, "split-list", split_list_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Splits a list of paths joined by the OS-specific list-separator, usually found in PATH or GOPATH environment variables.
  Returns an empty slice when passed an empty string.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("[String]")))

	ns.InternVar(env, "to-slash", to_slash_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Returns the result of replacing each separator character in path with a slash ('/') character.
  Multiple separators are replaced by multiple slashes.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))

	ns.InternVar(env, "volume-name", volume_name_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("path"))),
			`Returns leading volume name. Given "C:\foo\bar" it returns "C:" on Windows. Given "\\host\share\foo"
  returns "\\host\share". On other platforms it returns "".`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))
