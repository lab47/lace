...
func InternsOrThunks(env *Env, ns *Namespace) {
	...
	ns.InternVar(env, "create", create_,
		MakeMeta(
			...
			`Creates the named file with mode 0666 (before umask), truncating it if it already exists.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("File")))

	ns.InternVar(env, "cwd", cwd_,
		MakeMeta(
			NewListFrom(NewVectorFrom()),
			`Returns a rooted path name corresponding to the current directory. If the current directory can
  be reached via multiple paths (due to symbolic links), cwd may return any one of them.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("String")))
  ...
  *in* (in which case Joker's stdin will be redirected to the program's stdin) or the value returned by (lace.os/open).
  ...
  :err - string capturing stderr of the program (unless :stderr option was passed).`, "1.0"))
	...
	ns.InternVar(env, "exists?", isexists_,
		MakeMeta(
			...
			`Returns true if file or directory with the given path exists. Otherwise returns false.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("Boolean")))
  ...
  the associated file descriptor has mode O_RDONLY.`, "1.0").Plus(env, MakeKeyword("tag"), MakeString("File")))
