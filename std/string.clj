...
(defn ^Boolean ends-with?
  ...
  {:added "1.0"
  :go "strings.HasSuffix(s, substr), nil"}
  [^String s ^Stringable substr])

(defn ^Boolean starts-with?
  "True if s starts with substr."
  {:added "1.0"
  :go "strings.HasPrefix(s, substr), nil"}
  [^String s ^Stringable substr])
...
(defn ^String trim-left
  ...
  {:added "1.0"
  :go "strings.TrimLeftFunc(s, unicode.IsSpace), nil"}
  [^String s])
...
(defn ^String trim-right
  ...
  {:added "1.0"
  :go "strings.TrimRightFunc(s, unicode.IsSpace), nil"}
  [^String s])
...
(defn ^String escape
  ...
  {:added "1.0"
  :go "escape(_env, s, cmap)"}
  [^String s ^Callable cmap])

(defn ^Boolean includes?
  "True if s includes substr."
  {:added "1.0"
  :go "strings.Contains(s, substr), nil"}
  [^String s ^Stringable substr])
...
(defn ^String lower-case
  ...
  {:added "1.0"
  :go "strings.ToLower(s), nil"}
  [^Stringable s])

(defn ^String upper-case
  "Converts string to all upper-case."
  {:added "1.0"
  :go "strings.ToUpper(s), nil"}
  [^Stringable s])
...
(defn ^Regex re-quote
  ...
  {:added "1.0"
   :go "regexp.Compile(regexp.QuoteMeta(s))"}
  [^String s])
