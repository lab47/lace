...
func InternsOrThunks(env *Env, ns *Namespace) {
	...
	ns.InternVar(env, "decode-string", decode_string_,
		MakeMeta(
			...
			`Returns the bytes represented by the base64 string s.`, "1.0").Plus(env, MakeKeyword("tag"), String{S: "String"}))

	ns.InternVar(env, "encode-string", encode_string_,
		MakeMeta(
			NewListFrom(NewVectorFrom(MakeSymbol("s"))),
			`Returns the base64 encoding of s.`, "1.0").Plus(env, MakeKeyword("tag"), String{S: "String"}))
