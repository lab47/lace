...
(defn ^String format-double
  ...
  {:added "1.0"
  :go "strconv.FormatFloat(f, byte(fmt), prec, bitSize), nil"}
...
(defn ^String format-int
  ...
  {:added "1.0"
  :go "strconv.FormatInt(int64(i), base), nil"}
  [^Int i ^Int base])
...
(defn ^Boolean printable?
  ...
  {:added "1.0"
  :go "strconv.IsPrint(c), nil"}
  [^Char c])

(defn ^String itoa
  "Equivalent to (format-int i 10)."
  {:added "1.0"
  :go "strconv.Itoa(i), nil"}
  [^Int i])
...
(defn ^Int parse-int
  ...
  {:added "1.0"
  ;; TODO: 32-bit issue
  :go "!t, err := strconv.ParseInt(s, base, bitSize); _res := int(t)"}
...
(defn ^String quote-char-to-ascii
  ...
  {:added "1.0"
  :go "strconv.QuoteRuneToASCII(c), nil"}
  [^Char c])
...
(defn ^String quote-char-to-graphic
  ...
  {:added "1.0"
  :go "strconv.QuoteRuneToGraphic(c), nil"}
  [^Char c])
...
(defn ^String quote-to-graphic
  ...
  for non-ASCII characters and non-printable characters as defined by graphic?."
  {:added "1.0"
  :go "strconv.QuoteToGraphic(s), nil"}
  [^String s])
